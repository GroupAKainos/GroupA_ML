...
```bash
formality-api$ sam local invoke InferenceFunction --event events/event.json
```
...
```bash
formality-api$ sam logs -n InferenceFunction --stack-name formality-api --tail
```
...
```bash
aws cloudformation delete-stack --stack-name formality-api
```
